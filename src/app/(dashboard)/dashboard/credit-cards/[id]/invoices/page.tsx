...
export default function CreditCardInvoicesPage() {
  ...
  const fetchCreditCard = async () => {
    try {
      const data = await api.get<{ creditCard: CreditCard }>(
        `/credit-cards/${cardId}`,
      );
  ...
  const handlePaySubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const totalUnpaid = unpaidInvoices.reduce((sum, inv) => {
        const remaining = inv.totalPaid
          ? inv.amount - inv.totalPaid
          : inv.amount;
        return sum + remaining;
      }, 0);
      ...
      if (paymentAmount > totalUnpaid) {
        alert(
          `O valor não pode ser maior que o total pendente (${formatCurrency(totalUnpaid)})`,
        );
        setPaying(false);
        return;
      }
      ...
      unpaidInvoices.forEach((invoice, index) => {
        const remaining = invoice.totalPaid
          ? invoice.amount - invoice.totalPaid
          : invoice.amount;
        ...
        if (index === unpaidInvoices.length - 1) {
          itemPaymentAmount =
            Math.round((paymentAmount - totalCalculated) * 100) / 100;
        }
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (
        editAllParcels &&
        editingInvoice.installments &&
        editingInvoice.installments.total > 1
      ) {
        ...
        const relatedInvoices = invoices.filter((inv) => {
          ...
          const baseDesc = editingInvoice.description.replace(
            /\s*\(\d+\/\d+\)$/,
            "",
          );
          const invBaseDesc = inv.description.replace(/\s*\(\d+\/\d+\)$/, "");
          return (
            invBaseDesc === baseDesc &&
        ...
        const description =
          editingInvoice.installments && editingInvoice.installments.total > 1
    ...
    try {
      if (deleteAll) {
        ...
        const relatedInvoices = invoices.filter((inv) => {
          ...
          // Extrair a descrição base (sem o número da parcela)
          const baseDesc = confirmInvoice.description.replace(
            /\s*\(\d+\/\d+\)$/,
            "",
          );
          const invBaseDesc = inv.description.replace(/\s*\(\d+\/\d+\)$/, "");
        ...
        await Promise.all(
          relatedInvoices.map((inv) => api.delete(`/debts/${inv._id}`)),
        );
      } else {
  ...
  const handleAdjustmentSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const dueDateObj = new Date(
        year,
        month - 1,
        creditCard.dueDate || creditCard.bestPurchaseDay,
      );
      ...
      const newTotalInvoiceAmount = parseFloat(
      ...
      if (adjustmentDifference === 0) {
        alert(
          "O novo valor é igual ao valor atual. Não há necessidade de reajuste.",
        );
        setSubmittingAdjustment(false);
        return;
      }
  ...
  const purchases = invoices.filter(
    (inv) => !inv.description.includes("Reajuste de Fatura"),
  );
  const adjustments = invoices.filter((inv) =>
    inv.description.includes("Reajuste de Fatura"),
  );
  ...
  const paidAmount =
    adjustments.length > 0 ? paidPurchases + paidAdjustments : paidPurchases;
                    ...
                    <div>
                      ...
                      !invoice.paid ? (
                        <p className="text-xs text-gray-500 mt-1">
                          Restante:{" "}
                          {formatCurrency(invoice.amount - invoice.totalPaid)}
                        </p>
                      ) : (
                        ""
                      )}
                ...
                {addFormData.type === "installment" && (
                  <>
                    ...
                    <div>
                      ...
                      {addFormData.isTotalAmount &&
                        addFormData.installmentCount && (
                          <p className="mt-2 text-sm text-gray-600">
                            Cada parcela será de:{" "}
                            {formatCurrency(
                              parseFloat(addFormData.amount || "0") /
                                parseInt(addFormData.installmentCount),
                            )}
                          </p>
                        )}
                    </div>
          ...
          <div className="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
            <div className="mt-3">
              ...
              <div className="mb-4 p-3 bg-gray-50 rounded-md">
                ...
                <p className="text-sm text-gray-600 mb-1">
                  (Compras: {formatCurrency(totalPurchases)} + Reajustes:{" "}
                  {formatCurrency(adjustmentsAmount)})
                </p>
                <p className="text-sm text-gray-600">
                  Informe o valor total que será pago na fatura. O sistema
                  calculará a diferença automaticamente.
                </p>
              </div>
              <form onSubmit={handleAdjustmentSubmit} className="space-y-4">
                <div>
                  ...
                  />
                  <p className="mt-1 text-xs text-gray-500">
                    Este é o valor total que você vai pagar na fatura (incluindo
                    juros, IOF, descontos, etc.)
                  </p>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700">
                ...
                {adjustmentFormData.newTotalAmount && (
                  <div className="p-3 bg-blue-50 rounded-md space-y-2">
                    ...
                    <p className="text-sm text-gray-700">
                      ...
                      <span className="font-semibold">
                        {formatCurrency(
                          parseFloat(adjustmentFormData.newTotalAmount || "0"),
                        )}
                      </span>
                    </p>
                    {parseFloat(adjustmentFormData.newTotalAmount || "0") !==
                        ...
                        <p className="text-sm text-gray-700">
                          ...
                          <span
                            className={`text-lg font-bold ${
                              parseFloat(
                                adjustmentFormData.newTotalAmount || "0",
                              ) -
                                currentTotal >=
                              0
                                ? "text-blue-600"
            ...
            <div className="mt-3">
              ...
              <div className="mb-4 p-3 bg-yellow-50 rounded-md">
                <p className="text-sm text-gray-700 mb-2">
                  Esta compra possui{" "}
                  <strong>{confirmInvoice.installments?.total} parcelas</strong>
                  .
                </p>
                <p className="text-sm text-gray-700">
                  {confirmAction === "edit"
              ...
              <div className="space-y-3">
                ...
                >
                  <div className="font-semibold">
                    Todas as {confirmInvoice.installments?.total} parcelas
                  </div>
                ...
                return (
                  <>
                    <div className="text-sm text-gray-600 mb-4 space-y-1 p-3 bg-gray-50 rounded-md">
                      ...
                      {totalPaid > 0 && (
                        <p>
                          <strong>Já Pago:</strong> {formatCurrency(totalPaid)}
                        </p>
                      )}
                      <p>
                        <strong>Pendente:</strong> {formatCurrency(totalUnpaid)}
                      </p>
                    </div>
                        ...
                        <select
                          ...
                          onChange={(e) => {
                            if (e.target.value.startsWith("account-")) {
                              setPayFormData({
                                ...payFormData,
                                accountId: e.target.value.replace(
                                  "account-",
                                  "",
                                ),
                                walletId: "",
                ...
                editingInvoice.installments.total > 1 && (
                  <div className="mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded-md">
                    <p className="text-sm text-yellow-800">
                      ...
                      todas as {editingInvoice.installments.total} parcelas
                      desta compra.
                    </p>
                  </div>
                )}
