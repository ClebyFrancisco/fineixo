...
export default function SubscriptionModal({ isOpen, onClose }: SubscriptionModalProps) {
  ...
  const handleSubscribe = async (plan: 'monthly' | 'semiannual' | 'annual') => {
    try {
      setLoading(plan);
      const response = await api.post<{ sessionId: string; url: string }>('/subscriptions/checkout', {
        plan,
      });
            ...
            <div className="sm:flex sm:items-start">
              <div className="mt-3 text-center sm:mt-0 sm:text-left w-full">
                ...
                <p className="text-sm text-gray-500 mb-8">
                  Assine agora e tenha acesso completo a todas as funcionalidades do MyFinanceApp
                </p>
